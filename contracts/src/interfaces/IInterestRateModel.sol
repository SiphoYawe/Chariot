...
interface IInterestRateModel {
    ...
    function setParameters(uint256 optimalUtilisation, uint256 slope1, uint256 slope2, uint256 reserveFactor_) external;

    // -- Volatility premium functions (Phase 2) --
    function getVolatilityPremium(address collateralToken) external view returns (uint256);
    function getBorrowRateWithVolatility(uint256 utilisation, address collateralToken) external view returns (uint256);
